  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const [customers, deals] = await Promise.all([
        fetchAllStrapi(API_BASE_URL, '/api/customers?filters[channel_person][id][$notNull]=true&populate[channel_person][fields][0]=name&pagination[pageSize]=100'),
        fetchAllStrapi(API_BASE_URL, '/api/interactions?filters[is_deal][$eq]=true&populate[customer][fields][0]=name&pagination[pageSize]=100'),
      ]);
